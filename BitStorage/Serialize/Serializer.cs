...
namespace GgoSoft.Serialize
{
	...
	public static class Serializer
	{
		...
		public static void SerializeObject<T>(T obj, BitStorage storage, SerializerContext ctx)
		{
			...
			foreach (var field in metadata.FieldsInOrder)
			{
				...
				if (field.TypeResolution[0].IsPrimitive)
				{
					...
					//if (TryGetBigInteger(valueObject, out var bigInt))
					//{
				...
				{
					var list = EvaluateEnumerable(field, valueObject, 0);
					//var newObj = ProcessNode(field, 0, list);
					WriteEnumerable(storage, field, list, 0);
					int i = 10;
					//var action = CompileSerializer(obj.GetType(), field);
					//action(obj, storage); 
					Console.WriteLine();
				}
			}
		...
		private static List<object> ReadEnumerable<T>(BitStorageReader reader, FieldMetadata field, int depth) where T:struct
		{
			List<object> returnValue = [];
			var count = ReaderHelper<int>(reader, field, depth, 8);
			returnValue.Add(count);
			int newDepth = depth + 1;
			for(int i = 0; i < count; i++)
			{
				if (field.TypeResolution[newDepth].IsPrimitive)
				{
					var value = ReaderHelper<T>(reader, field, newDepth, 8);
					returnValue.Add(value);
				}
				else if (field.TypeResolution[newDepth].IsEnumerable)
				{
					returnValue.Add(ReadEnumerable<T>(reader, field, newDepth));
				}
				else
				{
		...
		static void WriteEnumerable(BitStorage storage, FieldMetadata field, object? valueObject, int depth)
		{
			if (valueObject == null)
			{
			...
			foreach (var value in enumerable)
			{
				...
				else
				{
					...
					{
						WriteEnumerable(storage, field, value, newDepth);
					}
					else
					{
						throw new SerializationException($"Was expecting primitive or enumerable on {field.Name} at depth {depth}");
		...
		static List<object> EvaluateEnumerable(FieldMetadata field, object? valueObject, int depth)
		{
			if(valueObject == null)
			{
				throw new SerializationException($"Wasn't expecting null as a value on {field.Name} at depth {depth}");
			...
			foreach(var value in enumerable)
			{
				...
				{
					returnValue.Add(EvaluateEnumerable(field, value, newDepth));
				}
				else
				{
					throw new SerializationException($"Was expecting primitive or enumerable on {field.Name} at depth {depth}");
...
object CreateAndPopulate(Type targetType, IReadOnlyList<FieldMetadata> fields, IDictionary<string, object?> values, IServiceProvider? services = null)
{
    ...
    return instance;
		...
		public static T? DeserializeObject<T>(BitStorageReader reader, SerializerContext ctx) where T : new()
		{
			...
			foreach (var field in metadata.FieldsInOrder)
			{
				...
				else
				{
					...
					//var list = ReadEnumerable(reader, field, 0);
					if (list is List<object> processingList)
					{
						var processedList = ProcessNode(field, 0, processingList);
					...
					//var list = reader.ReadEnumerable<int>(6 * 8, 8).ToList();
					int i = 10;
					Console.WriteLine();
				}
			}
		...
		{
			var t = fm.UnderlyingType.FieldType;// Nullable.GetUnderlyingType(fm.Property!.PropertyType) ?? fm.Property.PropertyType;
			int bitsRead = 0;
			if (t == typeof(int))
			{
